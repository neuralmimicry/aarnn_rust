#!/usr/bin/env python3
"""
Launch a mixed local Celegans Webots cluster:
...
Celegans network across all three nodes. The orchestrator/web UI therefore shows
...
DEFAULT_NETWORK_ID = "celegans_01"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "nodes against a single sharded Celegans Webots network."
        )
    )
    ...
    parser.add_argument(
        "--network-file",
        default=str(ROOT_DIR / "network_celegans.json"),
        help="Path to the Celegans snapshot JSON.",
    )
    parser.add_argument(
        "--config-file",
        default=str(ROOT_DIR / "webots_world/configs/config_celegans_webots.json"),
        help="Path to the Celegans Webots NetworkConfig JSON.",
    )
    parser.add_argument(
        "--world-file",
        default=str(ROOT_DIR / "webots_world/worlds/celegans_neuroworld.wbt"),
    ...
    parser.add_argument(
        "--logs-dir",
        default="",
        help="Directory for runtime logs. Default: logs/celegans_combo_webots-<timestamp>",
    )
    parser.add_argument(
        "--rust-ui-hidden",
        action="store_true",
...
def validate_paths(args: argparse.Namespace) -> tuple[Path, Path, Path]:
    ...
    if missing:
        ...
        raise LaunchError(f"Required Celegans assets are missing:\n{formatted}")
    return network_path, config_path, world_path


def main() -> int:
    args = parse_args()
    network_path, config_path, base_world = validate_paths(args)
    ...
    logs_dir = (
        ...
        if args.logs_dir
        else (ROOT_DIR / "logs" / f"celegans_combo_webots-{timestamp}").resolve()
    )
    ...
    try:
        ...
        orchestrator_port = (
            ...
            else find_free_port(DEFAULT_ORCHESTRATOR_PORT, reserved_ports)
        )
        if args.orchestrator_port > 0:
            reserved_ports.add(orchestrator_port)
        web_ui_port = (
            args.web_ui_port
            if args.web_ui_port > 0
            else find_free_port(DEFAULT_WEB_UI_PORT, reserved_ports)
        )
        if args.web_ui_port > 0:
            reserved_ports.add(web_ui_port)
        rust_node_port = find_free_port(args.node_port_start, reserved_ports)
        cli_node_1_port = find_free_port(rust_node_port + 7, reserved_ports)
        cli_node_2_port = find_free_port(cli_node_1_port + 7, reserved_ports)
        ...
        specs_json = network_specs_json(args.network_id, network_path, config_path)
        ...
        print("", flush=True)
        print(
            "The orchestrator web UI should show the combined Celegans network. "
