...
aarnn_container_workload_targets() {
    ...
    case "$workload" in
        ...
        standalone|orchestrator|node|stable-orchestrator|stable-node|desktop-ui) printf '%s' 'aarnn_rust' ;;
    esac
}
...
aarnn_container_workload_description() {
    ...
    case "$workload" in
        ...
        stable-orchestrator) printf '%s' 'Authenticated stable runtime orchestrator with live migration control' ;;
        stable-node) printf '%s' 'Authenticated stable runtime worker with live migration support' ;;
