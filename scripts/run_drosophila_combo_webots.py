#!/usr/bin/env python3
"""
Launch a mixed local Drosophila Webots cluster:
...
Drosophila network across all three nodes. The orchestrator/web UI therefore shows
...
DEFAULT_NETWORK_ID = "drosophila_01"
DEFAULT_ORCHESTRATOR_PORT = 50051
DEFAULT_WEB_UI_PORT = 8080
DEFAULT_NODE_PORT_START = 50070
DEFAULT_CONNECT_TIMEOUT = 300
DEFAULT_DISTRIBUTION_TIMEOUT = 1800
DEFAULT_DROSOPHILA_RETINA_WIDTH = 12
DEFAULT_DROSOPHILA_RETINA_HEIGHT = 8
DEFAULT_DROSOPHILA_CAMERA_WIDTH = 32
DEFAULT_DROSOPHILA_CAMERA_HEIGHT = 24
DEFAULT_DROSOPHILA_MAX_SENSORY = 34 + (
    4 * DEFAULT_DROSOPHILA_RETINA_WIDTH * DEFAULT_DROSOPHILA_RETINA_HEIGHT
)
DEFAULT_DROSOPHILA_MAX_HIDDEN = 20_000
DEFAULT_DROSOPHILA_MAX_OUTPUT = 48
DEFAULT_DROSOPHILA_HIDDEN_LAYER_WIDTH = 512
DEFAULT_DROSOPHILA_LONG_RANGE_POLICY = "fold"
DEFAULT_DROSOPHILA_WEIGHT_TRANSFORM = "sqrt"
DEFAULT_DROSOPHILA_MIN_SYN_COUNT = 1
...
def ensure_drosophila_assets(
    ...
    neurons_path = (ROOT_DIR / "data/drosophila/BANC v626/neurons.csv.gz").resolve()
    connections_path = (
        ROOT_DIR / "data/drosophila/BANC v626/connections_princeton.csv.gz"
    ).resolve()

    if args.rebuild_network or not network_path.is_file():
        missing_inputs = [
            path for path in (neurons_path, connections_path) if not path.is_file()
        ]
        if missing_inputs:
            formatted = "\n".join(f"  - {path}" for path in missing_inputs)
            raise LaunchError(
                "Cannot build Drosophila network snapshot because required BANC inputs "
                f"are missing:\n{formatted}"
            )
        print("Building Drosophila BANC network snapshot...", flush=True)
        run_checked(
            [
                "python3",
                str(ROOT_DIR / "scripts/build_drosophila_network_json.py"),
                "--neurons",
                str(neurons_path),
                "--connections",
                str(connections_path),
                "--template",
                str(ROOT_DIR / "network.json"),
                "--output",
                str(network_path),
                "--max-sensory",
                str(DEFAULT_DROSOPHILA_MAX_SENSORY),
                "--max-hidden",
                str(DEFAULT_DROSOPHILA_MAX_HIDDEN),
                "--max-output",
                str(DEFAULT_DROSOPHILA_MAX_OUTPUT),
                "--min-syn-count",
                str(DEFAULT_DROSOPHILA_MIN_SYN_COUNT),
                "--weight-transform",
                DEFAULT_DROSOPHILA_WEIGHT_TRANSFORM,
                "--hidden-layer-width",
                str(DEFAULT_DROSOPHILA_HIDDEN_LAYER_WIDTH),
                "--long-range-policy",
                DEFAULT_DROSOPHILA_LONG_RANGE_POLICY,
            ],
            cwd=ROOT_DIR,
            log_path=logs_dir / "build_drosophila_network.log",
        )

    if args.rebuild_assets or not config_path.is_file() or not world_path.is_file():
        print("Building Drosophila Webots assets...", flush=True)
        run_checked(
            [
                "python3",
                str(ROOT_DIR / "scripts/build_webots_drosophila_assets.py"),
                "--network",
                str(network_path),
                "--config",
                str(config_path),
                ...
                "1",
                "--eye-retina-width",
                str(DEFAULT_DROSOPHILA_RETINA_WIDTH),
                "--eye-retina-height",
                str(DEFAULT_DROSOPHILA_RETINA_HEIGHT),
                "--eye-camera-width",
                str(DEFAULT_DROSOPHILA_CAMERA_WIDTH),
                "--eye-camera-height",
                str(DEFAULT_DROSOPHILA_CAMERA_HEIGHT),
            ],
            cwd=ROOT_DIR,
            log_path=logs_dir / "build_drosophila_assets.log",
        )

    missing = [path for path in (network_path, config_path, world_path) if not path.is_file()]
    if missing:
        formatted = "\n".join(f"  - {path}" for path in missing)
        raise LaunchError(f"Required Drosophila assets are missing:\n{formatted}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "nodes against a single sharded Drosophila Webots network."
        )
    )
    ...
    parser.add_argument(
        "--network-file",
        default=str(ROOT_DIR / "network_drosophila.json"),
        help="Path to the Drosophila snapshot JSON.",
    )
    parser.add_argument(
        "--config-file",
        default=str(ROOT_DIR / "webots_world/configs/config_drosophila_webots.json"),
        help="Path to the Drosophila Webots NetworkConfig JSON.",
    )
    parser.add_argument(
        "--world-file",
        default=str(ROOT_DIR / "webots_world/worlds/drosophila_neuroworld.wbt"),
        help="Base Webots world file used to create the runtime world copy.",
    )
    parser.add_argument(
        "--rebuild-network",
        action="store_true",
        help="Force rebuilding `network_drosophila.json` from the local BANC dataset.",
    )
    parser.add_argument(
        "--rebuild-assets",
        action="store_true",
        help="Force rebuilding the Drosophila Webots config/world assets.",
    )
    parser.add_argument(
        "--orchestrator-port",
        type=int,
    ...
    parser.add_argument(
        "--logs-dir",
        default="",
        help="Directory for runtime logs. Default: logs/drosophila_combo_webots-<timestamp>",
    )
    parser.add_argument(
        "--rust-ui-hidden",
        action="store_true",
...
def main() -> int:
    ...
    logs_dir = (
        ...
        if args.logs_dir
        else (ROOT_DIR / "logs" / f"drosophila_combo_webots-{timestamp}").resolve()
    )
    logs_dir.mkdir(parents=True, exist_ok=True)

    network_path, config_path, base_world = ensure_drosophila_assets(args, logs_dir)
    ...
    try:
        ...
        print("", flush=True)
        print(
            "The orchestrator web UI should show the combined Drosophila network. "
