...
def _body_colour(i: int, total: int) -> Tuple[float, float, float]:
    """Larval-inspired tissue colours; segment boundaries are not pigment stripes."""
    t = i / max(1, total - 1)
    return 0.48 + 0.15 * t, 0.64 + 0.10 * t, 0.58 + 0.13 * t
...
def build_jaw_motors() -> str:
    ...
    for name, x_f in [("zebrafish_o_28_jaw", HEAD_LENGTH * 0.45),
        ...
        endPoint Solid {{
          ...
          immersionProperties [ ImmersionProperties {{ fluidName "nm_freshwater" }} ]
...
def build_proto(proto_name: str = "ZebrafishRobot") -> str:
    ...
    # Head neuromasts are attached to the existing body; tail landmarks remain on
    # their own physical segments rather than becoming an immobile duplicate fish.
    head_solid += webots_nodes([p for p in surface_details('fish') if p['anchor'] in
        {'segment_00', 'segment_01', 'segment_02'}], BODY_LENGTH, detail=True)
...
def build_world(proto_name: str = "ZebrafishRobot") -> str:
    return reference_world('zebrafish', proto_name, f'../protos/{proto_name}.proto')
