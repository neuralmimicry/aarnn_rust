...
pub extern "C" fn nm_init(config_json: *const c_char) -> c_int {
    ...
    if STATE.set(Mutex::new(state_instance)).is_err() {
        return -4;
    }
    0
}
...
pub extern "C" fn nm_set_port_by_index(start: usize, len: usize, data: *const f32) -> c_int {
    ...
    if start.checked_add(len).unwrap_or(usize::MAX) > guard.map.total_sensor_values() {
        return -2;
    }
    if data.is_null() {
        return -3;
...
pub extern "C" fn nm_get_port_by_index(start: usize, len: usize, out: *mut f32) -> c_int {
    ...
    if start.checked_add(len).unwrap_or(usize::MAX) > guard.map.total_actuator_values() {
        return -2;
    }
    if out.is_null() {
        return -3;
