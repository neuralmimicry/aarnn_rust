...
pub trait SensoryProvider {
    ...
    fn last_bands(&self) -> Option<&[f32]> { None }
    fn stop(&mut self) {}
...
#[cfg(feature = "ui")]
impl RandomProvider {
    pub fn new(num_sensory_neurons: usize, p: f32) -> Self { Self { num_sensory_neurons, p } }
}

#[cfg(feature = "ui")]
impl SensoryProvider for RandomProvider {
    fn next_spikes(&mut self) -> Vec<i8> {
        (0..self.num_sensory_neurons).map(|_| if fastrand::f32() < self.p { 1 } else { 0 }).collect()
    }
    fn set_num_sensory_neurons(&mut self, n_s: usize) { self.num_sensory_neurons = n_s; }
}
...
#[cfg(feature = "ui")]
impl ThetaProvider {
    pub fn new(num_sensory_neurons: usize, freq_hz: f32, duty: f32, phase_jitter: f32, dt_ms: f32) -> Self {
        let mut p = Self {
            num_sensory_neurons,
            freq_hz,
            duty,
...
impl SensoryProvider for ThetaProvider {
    ...
    fn set_dt(&mut self, dt_ms: f32) { self.dt_ms = dt_ms; }
}
...
impl BandMapper {
    pub fn new(num_sensory_neurons_target: usize, bands: usize) -> Self {
        ...
        for b in 0..bands {
            ...
            if b == bands-1 { end = num_sensory_neurons_target; }
    ...
    pub fn set_n_s(&mut self, n_s: usize) { *self = Self::new(n_s, self.bands); }
    pub fn ranges(&self) -> &[(usize, usize)] { &self.ranges }
}
...
impl AudioFileProvider {
    pub fn from_path(path: &std::path::Path, num_sensory_neurons: usize) -> anyhow::Result<Self> {
        ...
        let _sample_rate = codec_params.sample_rate.ok_or_else(|| anyhow::anyhow!("Unknown sample rate"))?;
                ...
                Ok(audio_buf) => {
                    ...
                    if chans == 1 {
                        ...
                        for i in 0..frames {
                            let mut s = 0.0f32;
                            let base = i * chans;
                            for c in 0..chans { s += data[base + c]; }
        ...
        if maxv > 0.0001 { for v in &mut pcm { *v /= maxv; } }
        ...
        Ok(Self { num_sensory_neurons, data: pcm, cursor: 0, win, hop, fft, scratch, buf, bands, last_bands, mapper })
    }

    fn next_window(&mut self) -> Vec<f32> {
        if self.data.is_empty() { return Vec::new(); }
        if self.cursor + self.win >= self.data.len() { self.cursor = 0; }
    ...
    fn compute_bands(&mut self, win: &[f32]) {
        ...
        for i in 0..self.win { self.buf[i] = Complex32::new(0.0, 0.0); }
        for i in 0..n {
            let w = 0.5 - 0.5*(2.0*std::f32::consts::PI*i as f32 / (self.win as f32 - 1.0)).cos();
        ...
        for k in 0..ny { mags[k] = self.buf[k].norm(); }
        ...
        for (bi, val) in bands.iter_mut().enumerate() {
            ...
            let mut acc = 0.0f32;
            let mut cnt = 0usize;
            for k in k0..k1 { acc += mags[k]; cnt+=1; }
        ...
        for i in 0..self.bands { bands[i] = (0.6*self.last_bands[i] + 0.4*(bands[i]/maxv)).min(1.0); }
...
impl SensoryProvider for AudioFileProvider {
    fn next_spikes(&mut self) -> Vec<i8> {
        ...
        for (b, &(start, end)) in self.mapper.ranges().iter().enumerate() {
            ...
            for i in start..end { if fastrand::f32() < p { out[i] = 1; } }
        }
        out
    }
    fn last_bands(&self) -> Option<&[f32]> { Some(&self.last_bands) }
    fn set_num_sensory_neurons(&mut self, n_s: usize) { self.num_sensory_neurons = n_s; self.mapper.set_n_s(n_s); }
}
...
impl MicrophoneProvider {
    pub fn new(num_sensory_neurons: usize) -> anyhow::Result<Self> {
        ...
        let supported = if let Some(c) = chosen { c } else { device
    ...
    fn take_window(&mut self) -> Vec<f32> {
        ...
        if let Ok(b) = self.buf.lock() {
            let n = b.len();
            if n >= self.win { b[n - self.win..n].to_vec() } else { Vec::new() }
        } else { Vec::new() }
    }
    ...
    fn compute_bands(&mut self, win: &[f32]) {
        ...
        let n = self.win.min(win.len());
        for i in 0..self.win { self.cbuf[i] = Complex32::new(0.0, 0.0); }
        for i in 0..n {
            let w = 0.5 - 0.5*(2.0*std::f32::consts::PI*i as f32 / (self.win as f32 - 1.0)).cos();
            self.cbuf[i].re = win[i] * w;
        }
        self.fft.process_with_scratch(&mut self.cbuf, &mut self.scratch);
        let ny = self.win/2;
        let mut mags = vec![0.0f32; ny];
        for k in 0..ny { mags[k] = self.cbuf[k].norm(); }
        ...
        for bi in 0..self.bands {
            ...
            let k1 = (f1.powf(2.0) * ny as f32).ceil() as usize;
            let k1 = k1.max(k0+1).min(ny);
            let mut acc = 0.0f32; let mut cnt = 0usize;
            for k in k0..k1 { acc += mags[k]; cnt += 1; }
...
impl SensoryProvider for MicrophoneProvider {
    fn next_spikes(&mut self) -> Vec<i8> {
        ...
        if win.len() < self.win/2 {
            ...
            for v in &mut self.last_bands { *v *= 0.9; }
            return vec![0i8; self.num_sensory_neurons];
        }
        self.compute_bands(&win);
        let mut out = vec![0i8; self.num_sensory_neurons];
    ...
    fn last_bands(&self) -> Option<&[f32]> { Some(&self.last_bands) }
    fn stop(&mut self) { self.stream = None; }
    fn set_num_sensory_neurons(&mut self, n_s: usize) { self.num_sensory_neurons = n_s; self.mapper.set_n_s(n_s); }
}
...
impl ImageFileProvider {
    pub fn from_path(path: &std::path::Path, num_sensory_neurons: usize) -> anyhow::Result<Self> {
        ...
        Ok(Self { num_sensory_neurons: num_sensory_neurons, gray, w, h, threshold: 0.5, invert: false, use_max: true })
    }

    fn resample_columns(&self, target: usize) -> Vec<f32> {
        // Collapse rows by mean, then resample columns by mean or max
        if self.w == 0 || self.h == 0 || target == 0 { return vec![0.0; target]; }
        let mut col_vals = vec![0.0f32; self.w];
        for x in 0..self.w {
            let mut acc = 0.0f32;
            for y in 0..self.h { acc += self.gray[y*self.w + x]; }
        ...
        for i in 0..target {
            ...
            if end <= start { end = (start + 1).min(self.w); }
            if self.use_max {
                let mut m = 0.0f32;
                for x in start..end { m = m.max(col_vals[x]); }
                out[i] = m;
            } else {
                let mut acc = 0.0f32; let mut cnt = 0usize;
                for x in start..end { acc += col_vals[x]; cnt+=1; }
...
impl VideoFileProvider {
    pub fn from_path(path: &std::path::Path, num_sensory_neurons: usize, loop_on_eof: bool) -> anyhow::Result<Self> {
        use opencv::prelude::*;
        let cap = opencv::videoio::VideoCapture::from_file(path.to_string_lossy().as_ref(), opencv::videoio::CAP_ANY)?;
        if !cap.is_opened()? { return Err(anyhow::anyhow!("Failed to open video")); }
        Ok(Self { num_sensory_neurons: num_sensory_neurons, cap, loop_on_eof, threshold: 0.5, invert: false, use_max: true })
    }
    ...
    fn read_gray(&mut self) -> anyhow::Result<(Vec<f32>, usize, usize)> {
        ...
        if frame.empty() { return Ok((Vec::new(), 0, 0)); }
        let size = frame.size()?; let w = size.width as usize; let h = size.height as usize;
        let channels = frame.channels();
        let data_u8 = frame.data_bytes()?;
        let mut out = vec![0.0f32; w*h];
        if channels == 1 {
            for i in 0..(w*h) { out[i] = data_u8[i] as f32 / 255.0; }
        } else {
            // Assume BGR
    ...
    fn resample_cols(gray: &[f32], w: usize, h: usize, target: usize, use_max: bool) -> Vec<f32> {
        if w == 0 || h == 0 || target == 0 { return vec![0.0; target]; }
        let mut col_vals = vec![0.0f32; w];
        for x in 0..w {
            let mut acc = 0.0f32;
            for y in 0..h { acc += gray[y*w + x]; }
        ...
        for i in 0..target {
            ...
            if end <= start { end = (start + 1).min(w); }
            if use_max {
                let mut m = 0.0f32;
                for x in start..end { m = m.max(col_vals[x]); }
                out[i] = m;
...
impl SensoryProvider for VideoFileProvider {
    fn next_spikes(&mut self) -> Vec<i8> {
        let (gray, w, h) = match self.read_gray() { Ok(t) => t, Err(_) => (Vec::new(), 0, 0) };
        if gray.is_empty() { return vec![0; self.num_sensory_neurons]; }
        let vals = Self::resample_cols(&gray, if w>0 {w} else {1}, if h>0 {h} else {gray.len()}, self.num_sensory_neurons, self.use_max);
        let thr = self.threshold.clamp(0.0, 1.0);
        vals.into_iter().map(|v|{
            let p = if self.invert { 1.0 - v } else { v };
            if p >= thr { 1 } else { 0 }
...
#[cfg(all(feature = "ui", feature = "video_input", target_arch = "aarch64"))]
impl VideoFileProvider {
    pub fn from_path(_path: &std::path::Path, _num_sensory_neurons: usize, _loop_on_eof: bool) -> anyhow::Result<Self> {
        Err(anyhow::anyhow!(
...
impl WebcamCaptureProvider {
    pub fn new(index: u32, num_sensory_neurons: usize) -> anyhow::Result<Self> {
        ...
        cam.open_stream()?;
        Ok(Self { num_sensory_neurons: num_sensory_neurons, cam, threshold: 0.5, invert: false, use_max: true })
    }
}
...
impl SensoryProvider for WebcamCaptureProvider {
    fn next_spikes(&mut self) -> Vec<i8> {
        let frame = match self.cam.frame() { Ok(f) => f, Err(_) => return vec![0; self.num_sensory_neurons] };
        let res = frame.resolution();
        let w = res.width() as usize; let h = res.height() as usize;
        ...
        if pixels == 0 || len == 0 {
            // leave as zeros
        } else if len == pixels { // GRAY8
            for i in 0..pixels { gray[i] = buf[i] as f32 / 255.0; }
        } else if len == pixels * 2 { // Likely YUYV422: [Y0 U Y1 V] per two pixels
            for p in 0..pixels {
                let base = (p / 2) * 4; // bytes per pair
                let y_idx = if (p & 1) == 0 { base } else { base + 2 };
                if y_idx < len { gray[p] = buf[y_idx] as f32 / 255.0; }
            }
        } else if len >= pixels * 3 { // Assume interleaved RGB/BGR with at least 3 bytes per pixel
        ...
        let thr = self.threshold.clamp(0.0,1.0);
        vals.into_iter().map(|v|{ let p = if self.invert {1.0 - v} else {v}; if p>=thr {1} else {0} }).collect()
    }
    fn stop(&mut self) { let _ = self.cam.stop_stream(); }
    fn set_num_sensory_neurons(&mut self, n_s: usize) { self.num_sensory_neurons = n_s; }
}
