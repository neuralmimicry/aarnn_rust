...
pub fn draw_network_diagram(path: &str, cfg: &NetworkConfig, _w: &WeightsOut) -> Result<()> {
    ...
    for i in 0..cfg.num_sensory_neurons.min(80) {
        ...
        chart.draw_series(std::iter::once(Circle::new((x_sensory, y), 3, BLUE.filled())))?;
    }
    // Hidden layers
    for l in 0..cfg.num_hidden_layers {
        let x = x_step * (2.0 + l as f32);
        for j in 0..cfg.num_hidden_per_layer_initial.min(40) {
            let y = 0.1 + 0.8 * (j as f32) / (cfg.num_hidden_per_layer_initial.min(40) as f32 + 1.0);
    ...
    for k in 0..cfg.num_output_neurons.min(20) {
        ...
        chart.draw_series(std::iter::once(Circle::new((x_output, y), 5, GREEN.filled())))?;
    }
    root.present()?;
    Ok(())
}
...
pub fn draw_spike_raster(path: &str, spikes_h: &Vec<Array2<i8>>, spikes_o: &Array2<i8>, _dt: f64) -> Result<()> {
    let h = 800u32;
    ...
    for (l, sp) in spikes_h.iter().enumerate() {
        ...
        chart.draw_series((0..steps).flat_map(|t| {
    ...
    // Avoid drawing mesh/ticks to prevent plotters usize key_points overflow on large ranges
    chart.draw_series((0..steps).flat_map(|t| {
        (0..n).filter_map(move |j| if spikes_o[(t, j)] != 0 { Some((t, j)) } else { None })
    }).map(|(t, j)| Circle::new((t, j), 1, BLACK.filled())))?;
...
pub fn draw_weight_histograms(path: &str, w: &WeightsOut, outputs_only: bool) -> Result<()> {
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption(
            if outputs_only { "Weight Histogram (Output)" } else { "Weight Histograms" },
            ("sans-serif", 24)
        )
        .margin(20)
    ...
    let weights: Vec<f64> = if outputs_only {
        ...
        for m in &w.w_hh_fwd { v.extend(m.iter().copied()); }
        for m in &w.w_hh_bwd { v.extend(m.iter().copied()); }
        v
    };
    if !weights.is_empty() {
        let w_min = 0.0f64; let w_max = 1.0f64;
        for &val in &weights { 
