...
cargo build --bin aarnn_rust --release --all-features
...
./target/release/aarnn_rust --orchestrator --brain-id cluster_master --grpc-addr 0.0.0.0:$ORCH_PORT "${CONFIG_ARG[@]}" "${NETWORK_ARG[@]}" > orchestrator.log 2>&1 &
PIDS=("$!")
