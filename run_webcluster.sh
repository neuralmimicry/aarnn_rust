...
cargo build --bin neuromorphic_demo --release --all-features
cargo build --bin web_ui --release --all-features
...
#./target/release/neuromorphic_demo --brain-id standalone --continuous > standalone.log 2>&1 &
#PIDS=("$!")
...
./target/release/neuromorphic_demo --orchestrator --brain-id cluster_master --grpc-addr 0.0.0.0:$ORCH_PORT "${CONFIG_ARG[@]}" "${NETWORK_ARG[@]}" > orchestrator.log 2>&1 &
PIDS=("$!")
...
./target/release/neuromorphic_demo --node --brain-id node_1 --grpc-addr 0.0.0.0:$NODE1_PORT --orchestrator-addr http://127.0.0.1:$ORCH_PORT > node_1.log 2>&1 &
PIDS+=("$!")
sleep 1
./target/release/neuromorphic_demo --node --brain-id node_2 --grpc-addr 0.0.0.0:$NODE2_PORT --orchestrator-addr http://127.0.0.1:$ORCH_PORT > node_2.log 2>&1 &
PIDS+=("$!")

echo "Start Web interface"
./target/release/web_ui --orchestrator http://127.0.0.1:$ORCH_PORT > webui.log 2>&1 &
PIDS+=("$!")
...
echo "The WebUI is available via browser at http://localhost:8080"
