...
fn main() {
    ...
    io_mapping.add_port(PortSpec::new("Sonar/Left", PortKind::Sensor, current_sensory_idx, 1)); current_sensory_idx += 1;
    io_mapping.add_port(PortSpec::new("Sonar/Right", PortKind::Sensor, current_sensory_idx, 1)); current_sensory_idx += 1;
    io_mapping.add_port(PortSpec::new("Accel", PortKind::Sensor, current_sensory_idx, 3)); current_sensory_idx += 3;
    io_mapping.add_port(PortSpec::new("Gyro", PortKind::Sensor, current_sensory_idx, 2)); current_sensory_idx += 2;
    io_mapping.add_port(PortSpec::new("GPS", PortKind::Sensor, current_sensory_idx, 3)); current_sensory_idx += 3;
    io_mapping.add_port(PortSpec::new("InertialRPY", PortKind::Sensor, current_sensory_idx, 3)); current_sensory_idx += 3;
    io_mapping.add_port(PortSpec::new("Foot/L", PortKind::Sensor, current_sensory_idx, 4)); current_sensory_idx += 4;
    io_mapping.add_port(PortSpec::new("Foot/R", PortKind::Sensor, current_sensory_idx, 4)); current_sensory_idx += 4;
    io_mapping.add_port(PortSpec::new("Bumper/L", PortKind::Sensor, current_sensory_idx, 2)); current_sensory_idx += 2;
    io_mapping.add_port(PortSpec::new("Bumper/R", PortKind::Sensor, current_sensory_idx, 2)); current_sensory_idx += 2;
    let mut current_actuator_idx = 0usize;
    io_mapping.add_port(PortSpec::new("RShoulderPitch", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("LShoulderPitch", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("Hands/R", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("Hands/L", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("LED/Chest", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("LED/RFoot", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("LED/LFoot", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("LED/FaceR", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("LED/FaceL", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("LED/EarR", PortKind::Actuator, current_actuator_idx, 1)); current_actuator_idx += 1;
    io_mapping.add_port(PortSpec::new("LED/EarL", PortKind::Actuator, current_actuator_idx, 1));

    // --- Runner configuration ---
    let lif_params = LIFParams::default();
    let stdp_params = STDPParams::default();
    let network_config = NetworkConfig { num_sensory_neurons: sensory_size, num_hidden_layers: 2, num_hidden_per_layer_initial: 16, num_output_neurons: output_size, ..Default::default() };
    let neuron_model = NeuronModel::Lif;
    let learning_rule = Learning::Stdp;
    let runner = Runner::new(lif_params, stdp_params, network_config, neuron_model, learning_rule);
    ...
    let mut runner_bridge = ExternalRunnerBridge::new(runner, io_mapping.clone(), sensor_adapter, actuator_sink, quantizer);
    ...
    for step_idx in 0..10 {
        ...
        let gyro_data = [0.02 * (step_idx as f32).cos(), -0.015 * (step_idx as f32).sin()];
        ...
        runner_bridge.actuator.get_port("RShoulderPitch", &mut output_value_buffer);
        println!("t={:.1} ms, spikes_out[0..{}] some={:?}", current_time_ms, runner_bridge.mapping.output_size.min(4), &runner_bridge.out_buf[0..4.min(runner_bridge.out_buf.len())]);
